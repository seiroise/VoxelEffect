...
//			Debug.Log("RunningAttack");
		...
		void AirAttack(){
			animator.SetInteger("Action", 1);
			animator.SetTrigger("AttackTrigger");
			rpgCharacterInputController.current.jumpInput = true;
		}
		...
		void Aiming(){
			if(rpgCharacterInputController.HasAimInput() || rpgCharacterInputController.inputAiming && (weapon == Weapon.RIFLE || weapon == Weapon.TWOHANDCROSSBOW || weapon == Weapon.TWOHANDBOW)){
				if(!isAiming){
					isAiming = true;
